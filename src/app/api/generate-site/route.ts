...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) return NextResponse.json({ ...(await buildFallback(clean, referenceUrl || '')), source: 'fallback-no-gemini-key', model: 'shader-v14' });
...
- O resultado tem de parecer diferente a cada nova geração.`;
    ...
    for (const model of MODELS) {
      ...
      return NextResponse.json({ ...(await parseProject(text, clean, referenceUrl || '')), source: 'gemini-shader', model, seed });
    }

    return NextResponse.json({ ...(await buildFallback(clean, referenceUrl || '')), source: 'fallback-after-gemini-error', model: 'shader-v14', error: errors.join('\n\n') });
  } catch (error) {
