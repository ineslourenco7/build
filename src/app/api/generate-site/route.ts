...
type Layout = 'fintech' | 'editorial' | 'booking' | 'saas';
type Intel = { niche: Niche; sub: string; layout: Layout; palette: Palette[] };
...
function paletteButtons(intel: Intel) {
  return `<div class="paletteBox reveal"><span>Escolhe uma paleta</span>${intel.palette.map((p, i) => `<button class="paletteBtn magnetic${i === 0 ? ' active' : ''}" data-accent="${p.accent}" data-accent2="${p.accent2}" data-glow="${p.glow}">${p.name}</button>`).join('')}</div>`;
}

function hero(intel: Intel) {
  const c = copy(intel);
  const visual = intel.layout === 'editorial'
    ? `<div class="editorialVisual"><div class="floatAsset"></div><div class="floatAsset"></div><div class="floatAsset"></div><span>Featured stories</span></div>`
    : intel.layout === 'booking'
      ? `<div class="bookingVisual"><div class="phone floatAsset"><b>Hoje</b><p>14:30 · Serviço Premium</p><p>16:00 · Consulta</p><button class="magnetic">Confirmar</button></div><div class="swatches"><i></i><i></i><i></i></div></div>`
      : `<div class="productVisual"><div class="metric animatedBorder"><small>${intel.niche === 'trading' ? 'Volume mensal' : 'Workflows'}</small><strong>${intel.niche === 'trading' ? '$42.8M' : '1.8M'}</strong><em>+42% crescimento</em></div><div class="chart"><i></i><i></i><i></i><i></i><i></i><i></i></div><div class="signal floatAsset animatedBorder"><b>${intel.niche === 'trading' ? 'Risk score' : 'Automation health'}</b><strong>${intel.niche === 'trading' ? 'A+' : '94%'}</strong></div></div>`;
  return `<section class="hero sectionTransition"><div class="heroCopy reveal"><p class="eyebrow">${c.eyebrow}</p><h1>${c.title}</h1><p class="lead">${c.sub}</p><div class="actions"><button class="primary magnetic" data-target="product">${c.cta}</button><button class="secondary magnetic" data-target="pricing">Ver planos</button></div>${paletteButtons(intel)}</div><div class="heroCard reveal parallaxLayer animatedBorder" data-parallax="0.14">${visual}</div></section>`;
}

function features(intel: Intel) {
  const items = intel.niche === 'photography' ? ['Galerias editoriais', 'Booking de sessões', 'Entrega privada', 'Pacotes premium'] : intel.niche === 'beauty' ? ['Agenda online', 'Serviços e preços', 'Clientes recorrentes', 'Pagamentos'] : intel.sub.includes('copy') ? ['Perfis de traders', 'Risk score', 'Copy automation', 'Analytics'] : intel.niche === 'trading' ? ['Challenges', 'Risk rules', 'Payouts', 'Trader portal'] : ['Workflows', 'Client portal', 'Analytics', 'Automations'];
  return `<section id="product" class="section sectionTransition"><div class="sectionHead reveal"><p class="eyebrow">${intel.sub}</p><h2>${intel.niche === 'photography' ? 'Um portfólio que vende antes da primeira chamada.' : intel.niche === 'beauty' ? 'Uma experiência pensada para transformar visitas em marcações.' : intel.niche === 'trading' ? 'Performance, confiança e operação no mesmo produto.' : 'Uma plataforma clara, rápida e pronta para crescer.'}</h2><p>${intel.niche === 'trading' ? 'Dados, risco, perfis e automações apresentados com linguagem visual institucional.' : 'Conteúdo, navegação e interações organizados para criar confiança e conversão.'}</p></div><div class="bento">${items.map((item, index) => `<article class="bentoCard reveal animatedBorder" style="--stagger:${index}"><div class="icon">${index + 1}</div><h3>${item}</h3><p>${intel.niche === 'trading' ? 'Camada criada para aumentar transparência e reduzir fricção na decisão.' : 'Secção desenhada para dar contexto, clareza e valor percebido.'}</p></article>`).join('')}</div></section>`;
}
...
function app(intel: Intel) {
  ...
  return `<section id="app" class="appSection sectionTransition"><div class="sectionHead reveal"><p class="eyebrow">Área interna</p><h2>${intel.niche === 'photography' ? 'Gestão de clientes, galerias e entregas.' : intel.niche === 'beauty' ? 'Agenda, clientes e serviços sempre organizados.' : intel.niche === 'trading' ? 'Painel operacional para traders e métricas.' : 'Dashboard para gerir produto, clientes e automações.'}</h2><p>Uma área interna navegável com páginas, dados e estados simulados para mostrar como o produto funcionaria.</p></div><div class="appShell reveal parallaxLayer animatedBorder" data-parallax="0.06"><aside>${pages.map((page, index) => `<button class="appTab${index === 0 ? ' active' : ''}" data-app-page="${page.toLowerCase()}">${page}</button>`).join('')}</aside><div class="appMain">${pages.map((page, index) => `<section class="appPage${index === 0 ? ' active' : ''}" data-app-view="${page.toLowerCase()}"><div class="appTop"><span>${page}</span><button class="magnetic">Novo</button></div><div class="appStats">${stats.map((stat, statIndex) => `<article class="animatedBorder" style="--stagger:${statIndex}"><small>${stat.split(' ')[0]}</small><strong>${stat.split(' ').slice(1).join(' ')}</strong></article>`).join('')}</div><div class="appTable"><div><b>${page} item</b><span>Active</span><em>Updated now</em></div><div><b>${page} flow</b><span>Pending</span><em>12 min ago</em></div><div><b>${page} report</b><span>Synced</span><em>Today</em></div></div></section>`).join('')}</div></div></section>`;
}

function pricing() {
  return `<section id="pricing" class="section sectionTransition"><div class="sectionHead reveal"><p class="eyebrow">Planos</p><h2>Escolhe o nível certo para lançar.</h2></div><div class="pricing"><article class="price reveal animatedBorder" style="--stagger:0"><p>Launch</p><h3>€490</h3><span>Landing premium, analytics e formulário</span><button class="magnetic" data-target="contact">Escolher</button></article><article class="price featured reveal animatedBorder" style="--stagger:1"><div class="badge">Popular</div><p>Growth</p><h3>€1.490</h3><span>Website multi-page, área interna e automações</span><button class="magnetic" data-target="contact">Começar</button></article><article class="price reveal animatedBorder" style="--stagger:2"><p>Scale</p><h3>Custom</h3><span>Produto completo, integrações e área cliente</span><button class="magnetic" data-target="contact">Falar</button></article></div></section>`;
}

function contact() {
  return `<section id="contact" class="contact sectionTransition"><div class="reveal"><p class="eyebrow">Próximo passo</p><h2>Vamos transformar a visão num produto vendável.</h2><p>Envia o objetivo, referências e conteúdos. A experiência pode evoluir para website, dashboard, booking ou produto completo.</p></div><form id="contactForm" class="reveal animatedBorder"><input placeholder="Nome" required/><input type="email" placeholder="Email" required/><textarea placeholder="O que queres construir?" required></textarea><button class="primary magnetic" type="submit">Enviar pedido</button><p id="successMessage">Pedido recebido. Entraremos em contacto.</p></form></section>`;
}
